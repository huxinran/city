export enum Resident {
    FARMER = "Farmer",
    WORKER = "Worker",
    ARTISAN = "Artisan",
    SCHOLAR = "Scholar",
    ENTREPRENEUR = "Entrepreneur",
    MAGNATE = "Magnate",
}

export enum CityName {
    ANRELIA = "Anrelia",
    MINTAKA = "Mintaka",
    JINLIN = "Jinlin",
    COLUMBIA = "Columbia",
    SOLARA = "Solara",
}

export enum HouseType {
    COTTAGE = "Cottage",
    TENEMENT = "Tenement",
    HOUSE = "House",
    VILLA = "Villa",
    MANSION = "Mansion",
    ESTATE = "Estate",
}

export enum Resource {
    APPLE = "Apple",
    BANANA = "Banana",
    BERRY = "Berry",
    BOOT = "Boot",
    BRANDY = "Brandy",
    BREAD = "Bread",
    BRICK = "Brick",
    CABBAGE = "Cabbage",
    CANDLE = "Candle",
    CHEESE = "Cheese",
    CIDER = "Cider",
    CIGAR = "Cigar",
    CLAY = "Clay",
    COAL = "Coal",
    COCOA = "Cocoa",
    CONCRETE = "Concrete",
    CORN = "Corn",
    COTTON = "Cotton",
    EGG = "Egg",
    FEATHER_HAT = "Feather Hat",
    FISH = "Fish",
    FLOUR = "Flour",
    FUR = "Fur",
    GEM = "Gem",
    GLASS = "Glass",
    GLASS_BOTTLE = "Glass Bottle",
    HORSE = "Horse",
    GOLD = "Gold",
    GRAPE = "Grape",
    CLAM = "Clam",
    DUMPLINGS = "Dumplings",
    DYE = "Dye",
    ICE_CREAM = "Ice Cream",
    IRON = "Iron",
    INK = "Ink",
    JAM = "Jam",
    MARBLE = "Marble",
    MACHINE_PARTS = "Machine Parts",
    MEDICINE = "Medicine",
    MELON = "Melon",
    MILK = "Milk",
    OIL = "Oil",
    OLIVE = "Olive",
    ORANGE = "Orange",
    ONION = "Onion",
    PAINTINGS = "Paintings",
    PAPER = "Paper",
    FURNITURE = "Furniture",
    JEWELRY = "Jewelry",
    IRON_ORE = "Iron Ore",
    GOLD_ORE = "Gold Ore",
    PANT = "Pant",
    PERFUME = "Perfume",
    PICKLES = "Pickles",
    TIMBER = "Timber",
    TOOL = "Tool",
    PORK = "Pork",
    POTATO = "Potato",
    POTTERY = "Pottery",
    PUMPKIN = "Pumpkin",
    SALT = "Salt",
    SAND = "Sand",
    SAUSAGE = "Sausage",
    SHIRT = "Shirt",
    STONE_BLOCKS = "Stone Blocks",
    SOYBEAN = "Soybean",
    SOAP = "Soap",
    STATUE = "Statue",
    STEEL = "Steel",
    STONE = "Stone",
    SUSHI = "Sushi",
    SUGAR_CANE = "Sugar Cane",
    RICE = "Rice",
    RUBBER= "Rubber",
    RUM = "Rum",
    TOBACCO = "Tobacco",
    TOMATO = "Tomato",
    WAX = "Wax",
    WATCH = "Watch",
    WEAPON = "Weapon",
    WHEAT = "Wheat",
    WINDOW = "Window",
    WINE = "Wine",
    WOOD = "Wood",
    WOOL = "Wool",
    // --- Jinlin (East Asian city) ---
    TEA = "Tea",
    SILK = "Silk",
    TOFU = "Tofu",
    PORCELAIN = "Porcelain",
    NOODLE = "Noodle",
    // --- Columbia (American frontier city) ---
    LEATHER = "Leather",
    // --- Solara (African / tropical city) ---
    COPPER = "Copper",
    INCENSE = "Incense",
    IVORY = "Ivory",
    // --- Mintaka (polar city) ---
    SMOKED_FISH = "Smoked Fish",
    FUR_COAT = "Fur Coat",
    DEER = "Deer",
    WHALE_OIL = "Whale Oil",
    // --- Shared agricultural input ---
    FERTILIZER = "Fertilizer",
}

export enum ServiceType {
    WATER = "Water",
    FIRE = "Fire",
    POLICE = "Police",
    SCHOOL = "School",
    MARKET = "Market",
    TAVERN = "Tavern",
    CHURCH = "Church",
    HEALTH = "Health",
    JUSTICE = "Justice",
    ENGINEER = "Engineering",
}

export enum ProductionStatus {
    READY = "Ready",
    WAITING_DELIVERY = "Waiting Delivery",
    IN_PROGRESS = "In Progress",
    FINISHED = "Finished",
    WAITING_PICK_UP = "Waiting Pick Up"
}

export enum ShippingTaskType {
    READY = "Ready",
    PICKING_UP = "Picking Up",
    DELIVERING = "Delivering",
    STOCKING = "Stocking",
    RETURNING = "Returning",
}

export enum BuildingType {
    APPLE_ORCHARD = "Apple Orchard",
    BANANA_PLANTATION = "Banana Plantation",
    BERRY_GROVE = "Berry Grove",
    BRANDY_DISTILLERY = "Brandy Distillery",
    BAKERY = "Bakery",
    BRICKYARD = "Brickyard",
    CABBAGE_PATCH = "Cabbage Patch",
    CANDLE_MANUFACTORY = "Candle Manufactory",
    CREAMERY = "Creamery",
    CIDERY = "Cidery",
    CIGAR_FACTORY = "Cigar Factory",
    CLAY_PIT = "Clay Pit",
    COAL_KILN = "Coal Kiln",
    CONCRETE_PLANT = "Concrete Plant",
    SCULPTOR = "Sculptor",
    COCOA_PLANT = "Cocoa Plant",
    CORN_FIELD = "Corn Field",  
    COTTON_FIELD = "Cotton Field",
    CHICKEN_COOP = "Chicken Coop",
    FISHERY = "Fishery",
    HORSE_FARM = "Horse Farm",
    TRAPLINE = "Trapline",
    GEM_MINE = "Gem Mine",
    GLASSWORKS = "Glassworks",
    BOTTLEWORKS = "Bottleworks",
    GLAZIER = "Glazier",
    GOLD_MINE = "Gold Mine",
    VINEYARD = "Vineyard",
    WINDMILL = "Windmill",
    PAPER_MILL = "Paper Mill",
    INK_WORKSHOP = "Ink Workshop",
    DYE_WORKSHOP = "Dye Workshop",
    APOTHECARY = "Apothecary",
    PICKLERY = "Picklery",
    DUMPLING_KITCHEN = "Dumpling Kitchen",
    SUSHI_BAR = "Sushi Bar",
    ICE_CREAMERY = "Ice Creamery",
    SOAP_WORKS = "Soap Works",
    PERFUMERY = "Perfumery",
    MACHINE_SHOP = "Machine Shop",
    WATCHMAKER = "Watchmaker",
    PAINTER_STUDIO = "Painter Studio",
    PIG_FARM = "Pig Farm",
    LUMBER_HUT = "Lumber Hut",
    WHEAT_FARM = "Wheat Farm",
    IRON_MINE = "Iron Mine",
    PRESERVE_SHOP = "Preserve Shop",
    MELON_GARDEN = "Melon Garden",
    DAIRY_FARM = "Dairy Farm",
    OLIVE_GROVE = "Olive Grove",
    ORANGE_ORCHARD = "Orange Orchard",
    ONION_FIELD = "Onion Field",
    PANT_SHOP = "Pant Shop",
    SHIRT_SHOP = "Shirt Shop",
    BOOT_SHOP = "Boot Shop",
    FORGE = "Forge",
    GOLDSMITH = "Goldsmith",
    TOOLSMITH = "Toolsmith",
    JEWELER = "Jeweler",
    CARPENTER_SHOP = "Carpenter Shop",
    WINERY = "Winery",
    OIL_PRESS = "Oil Press",
    RUM_DISTILLERY = "Rum Distillery",
    SAWMILL = "Sawmill",
    POTATO_FARM = "Potato Farm",
    POTTERY_SHOP = "Pottery Shop",
    PUMPKIN_PATCH = "Pumpkin Patch",
    SALTERN = "Saltern",
    SAND_PIT = "Sand Pit",
    BUTCHERY = "Butchery",
    MASON_SHOP = "Mason Shop",
    SOYBEAN_FARM = "Soybean Farm",
    STEELWORKS = "Steelworks",
    STONE_QUARRY = "Stone Quarry",
    SUGAR_CANE_PLANTATION = "Sugar Cane Plantation",
    RICE_PADDY = "Rice Paddy",
    RUBBER_PLANTATION = "Rubber Plantation",
    TOBACCO_PLANTATION = "Tobacco Plantation",
    TOMATO_FIELD = "Tomato Field",
    APIARY = "Apiary",
    SHEEP_FARM = "Sheep Farm",
    HOUSE = "House",
    WELL = "Well",
    FIRE_STATION = "Fire Station",
    POLICE_STATION = "Police Station",
    SCHOOL = "School",
    MARKETPLACE = "Marketplace",
    TAVERN = "Tavern",
    CHAPEL = "Chapel",
    CLINIC = "Clinic",
    COURTHOUSE = "Courthouse",
    ENGINEER_STATION = "Engineer Station",
    WAREHOUSE = "Warehouse",
    SHIPYARD = "Shipyard",
    DOCK = "Dock",
    ROAD = "Road",
    DELETE = "Delete",
    // --- Jinlin (East Asian) buildings ---
    TEA_GARDEN = "Tea Garden",
    SILK_FARM = "Silk Farm",
    TOFU_SHOP = "Tofu Shop",
    KILN = "Kiln",
    NOODLE_SHOP = "Noodle Shop",
    // --- Columbia (American frontier) buildings ---
    TANNERY = "Tannery",
    // --- Solara (African / tropical) buildings ---
    COPPER_MINE = "Copper Mine",
    INCENSE_GROVE = "Incense Grove",
    IVORY_CAMP = "Ivory Camp",
    // --- Mintaka (polar) buildings ---
    REINDEER_FARM = "Reindeer Farm",
    FUR_WORKSHOP = "Fur Workshop",
    SMOKEHOUSE = "Smokehouse",
    WHALING_POST = "Whaling Post",
    // --- Shared utility ---
    COMPOST_PIT = "Compost Pit",
    UNIVERSITY = "University",
}

// "Land" is the general term for the buildable ground types (grass, dirt, sand);
// water is the non-buildable terrain. Trees and rocks are Features, not terrain.
export enum Terrain {
    GRASS = "Grass",
    SAND = "Sand",
    WATER = "Water",
    DIRT = "Dirt",
}

// Natural features that sit on top of a land tile, like a building does: trees
// (for lumber / foraging) and rock outcrops (for mining). They coexist with the
// terrain underneath and are removable with the Delete tool.
export enum Feature {
    TREE = "Tree",
    ROCK = "Rock",
    BUSH = "Bush",
}

export enum ShipType {
    CARGO = "Cargo",
    CLIPPER = "Clipper",
    GRAND = "Grand"
}

// Resources and buildings exclusive to each themed city.
// Excluded from the resource panel and build palette when playing a different city.
export const CITY_EXCLUSIVE_RESOURCES: Partial<Record<CityName, Resource[]>> = {
    [CityName.JINLIN]:   [Resource.TEA, Resource.SILK, Resource.TOFU, Resource.PORCELAIN, Resource.NOODLE],
    [CityName.COLUMBIA]: [Resource.LEATHER],
    [CityName.SOLARA]:   [Resource.COPPER, Resource.INCENSE, Resource.IVORY],
    [CityName.MINTAKA]:  [Resource.SMOKED_FISH, Resource.FUR_COAT, Resource.DEER, Resource.WHALE_OIL],
}

export const CITY_EXCLUSIVE_BUILDINGS: Partial<Record<CityName, BuildingType[]>> = {
    [CityName.JINLIN]:   [BuildingType.TEA_GARDEN, BuildingType.SILK_FARM, BuildingType.TOFU_SHOP, BuildingType.KILN, BuildingType.NOODLE_SHOP],
    [CityName.COLUMBIA]: [BuildingType.TANNERY],
    [CityName.SOLARA]:   [BuildingType.COPPER_MINE, BuildingType.INCENSE_GROVE, BuildingType.IVORY_CAMP],
    [CityName.MINTAKA]:  [BuildingType.REINDEER_FARM, BuildingType.FUR_WORKSHOP, BuildingType.SMOKEHOUSE, BuildingType.WHALING_POST],
}
