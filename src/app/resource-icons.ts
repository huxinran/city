import { Resource } from './sim/types'

// Shared resource icon lookup: a cute PNG when art exists, else an emoji.

const RESOURCE_ICON_BASE = 'assets/used/resources/'
const RESOURCE_ICON_FILES: { [key: string]: string } = {
  [Resource.APPLE]: 'apple.png',
  [Resource.BANANA]: 'banana.png',
  [Resource.BERRY]: 'berry.png',
  [Resource.BRANDY]: 'brandy.png',
  [Resource.BREAD]: 'bread.png',
  [Resource.BRICK]: 'brick.png',
  [Resource.CABBAGE]: 'cabbage.png',
  [Resource.CANDLE]: 'candle.png',
  [Resource.CHEESE]: 'cheese.png',
  [Resource.CIDER]: 'cider.png',
  [Resource.CIGAR]: 'cigar.png',
  [Resource.CLAY]: 'clay.png',
  [Resource.COAL]: 'coal.png',
  [Resource.COCOA]: 'cocoa.png',
  [Resource.CONCRETE]: 'concrete.png',
  [Resource.CORN]: 'corn.png',
  [Resource.COTTON]: 'cotton.png',
  [Resource.EGG]: 'egg.png',
  [Resource.FEATHER_HAT]: 'feather-hat.png',
  [Resource.FISH]: 'fish.png',
  [Resource.FLOUR]: 'flour.png',
  [Resource.FUR]: 'fur.png',
  [Resource.GEM]: 'gem.png',
  [Resource.GLASS]: 'glass.png',
  [Resource.GLASS_BOTTLE]: 'glass-bottle.png',
  [Resource.GOLD]: 'gold.png',
  [Resource.HORSE]: 'horse.png',
  [Resource.GRAPE]: 'grape.png',
  [Resource.CLAM]: 'clam.png',
  [Resource.DUMPLINGS]: 'dumplings.png',
  [Resource.DYE]: 'dye.png',
  [Resource.ICE_CREAM]: 'ice-cream.png',
  [Resource.INK]: 'ink.png',
  [Resource.IRON]: 'iron.png',
  [Resource.JAM]: 'jam.png',
  [Resource.MARBLE]: 'marble.png',
  [Resource.MACHINE_PARTS]: 'machine-parts.png',
  [Resource.MEDICINE]: 'medicine.png',
  [Resource.MELON]: 'melon.png',
  [Resource.MILK]: 'milk.png',
  [Resource.OIL]: 'oil.png',
  [Resource.OLIVE]: 'olive.png',
  [Resource.ORANGE]: 'orange.png',
  [Resource.ONION]: 'onion.png',
  [Resource.PAINTINGS]: 'paintings.png',
  [Resource.PAPER]: 'paper.png',
  [Resource.FURNITURE]: 'furniture.png',
  [Resource.JEWELRY]: 'jewelry.png',
  [Resource.IRON_ORE]: 'iron-ore.png',
  [Resource.GOLD_ORE]: 'gold-ore.png',
  [Resource.PANT]: 'pant.png',
  [Resource.PERFUME]: 'perfume.png',
  [Resource.PICKLES]: 'pickles.png',
  [Resource.SHIRT]: 'shirt.png',
  [Resource.BOOT]: 'boot.png',
  [Resource.TIMBER]: 'timber.png',
  [Resource.TOOL]: 'tool.png',
  [Resource.PORK]: 'pork.png',
  [Resource.POTATO]: 'potato.png',
  [Resource.POTTERY]: 'pottery.png',
  [Resource.PUMPKIN]: 'pumpkin.png',
  [Resource.SALT]: 'salt.png',
  [Resource.SAND]: 'sand.png',
  [Resource.SAUSAGE]: 'sausage.png',
  [Resource.STONE_BLOCKS]: 'stone-blocks.png',
  [Resource.SOYBEAN]: 'soybean.png',
  [Resource.SOAP]: 'soap.png',
  [Resource.STATUE]: 'statue.png',
  [Resource.STEEL]: 'steel.png',
  [Resource.STONE]: 'stone.png',
  [Resource.SUSHI]: 'sushi.png',
  [Resource.SUGAR_CANE]: 'sugar-cane.png',
  [Resource.RICE]: 'rice.png',
  [Resource.RUBBER]: 'rubber.png',
  [Resource.RUM]: 'rum.png',
  [Resource.TOBACCO]: 'tobacco.png',
  [Resource.TOMATO]: 'tomato.png',
  [Resource.WAX]: 'wax.png',
  [Resource.WATCH]: 'watch.png',
  [Resource.WEAPON]: 'weapon.png',
  [Resource.WHEAT]: 'wheat.png',
  [Resource.WINDOW]: 'window.png',
  [Resource.WINE]: 'wine.png',
  [Resource.WOOD]: 'wood.png',
  [Resource.WOOL]: 'wool.png',
  [Resource.TEA]: 'tea.png',
  [Resource.SILK]: 'silk.png',
  [Resource.TOFU]: 'tofu.png',
  [Resource.PORCELAIN]: 'porcelain.png',
  [Resource.NOODLE]: 'noodle.png',
  [Resource.LEATHER]: 'leather.png',
  [Resource.COPPER]: 'copper.png',
  [Resource.INCENSE]: 'incense.png',
  [Resource.IVORY]: 'ivory.png',
  [Resource.DEER]: 'deer.png',
  [Resource.SMOKED_FISH]: 'smoked-fish.png',
  [Resource.FUR_COAT]: 'fur-coat.png',
  [Resource.WHALE_OIL]: 'whale-oil.png',
  [Resource.FERTILIZER]: 'fertilizer.png',
}

const RESOURCE_EMOJI: { [key: string]: string } = {
  [Resource.APPLE]:      '🍎', [Resource.BANANA]:     '🍌', [Resource.BERRY]:      '🍓',
  [Resource.BRANDY]:     '🍾', [Resource.BREAD]:      '🥖', [Resource.BRICK]:      '🧱',
  [Resource.CABBAGE]:    '🥬', [Resource.CANDLE]:     '🕯️', [Resource.CHEESE]:     '🧀',
  [Resource.CIDER]:      '🍺', [Resource.CIGAR]:      '🚬', [Resource.CLAY]:       '🟤',
  [Resource.COAL]:       '⚫', [Resource.COCOA]:      '🍫', [Resource.CONCRETE]:   '🏗️',
  [Resource.CORN]:       '🌽', [Resource.COTTON]:     '🪡', [Resource.EGG]:        '🥚',
  [Resource.FEATHER_HAT]:'🎩',
  [Resource.FISH]:       '🐟', [Resource.FLOUR]:      '🥣', [Resource.FUR]:        '🦊',
  [Resource.FURNITURE]:  '🪑', [Resource.GEM]:        '💎',
  [Resource.GLASS]:      '🫧', [Resource.GOLD]:       '🪙', [Resource.GOLD_ORE]:   '🏅',
  [Resource.GLASS_BOTTLE]:'🍾',
  [Resource.GRAPE]:      '🍇', [Resource.HORSE]:      '🐴',
  [Resource.CLAM]:       '🦪', [Resource.DUMPLINGS]:  '🥟', [Resource.DYE]:        '🎨',
  [Resource.ICE_CREAM]:  '🍨', [Resource.INK]:        '🖋️',
  [Resource.IRON]:       '⚙️', [Resource.IRON_ORE]:   '⛏️', [Resource.JAM]:        '🫙',
  [Resource.JEWELRY]:    '💍', [Resource.MARBLE]:     '🪨', [Resource.MELON]:      '🍉', [Resource.MILK]:       '🥛',
  [Resource.MACHINE_PARTS]:'🔩', [Resource.MEDICINE]: '💊',
  [Resource.OIL]:        '🫗', [Resource.OLIVE]:      '🫒', [Resource.ONION]:      '🧅',
  [Resource.ORANGE]:     '🍊', [Resource.PAINTINGS]:  '🖼️', [Resource.PANT]:       '👖',
  [Resource.PAPER]:      '📄', [Resource.PERFUME]:    '🧴', [Resource.PICKLES]:    '🥒', [Resource.PORK]:       '🥩',
  [Resource.POTATO]:     '🥔', [Resource.POTTERY]:    '🏺', [Resource.PUMPKIN]:    '🎃',
  [Resource.RICE]:       '🍚', [Resource.RUBBER]:     '🌴', [Resource.RUM]:        '🥃',
  [Resource.SALT]:       '🧂', [Resource.SAND]:       '🏜️', [Resource.SAUSAGE]:    '🌭',
  [Resource.SHIRT]:      '👕', [Resource.BOOT]:       '👢',
  [Resource.STONE_BLOCKS]:      '🪨', [Resource.SOYBEAN]:    '🫘', [Resource.SOAP]:       '🧼',
  [Resource.STATUE]:     '🗿',
  [Resource.STEEL]:      '⚒️', [Resource.STONE]:      '🪨', [Resource.SUSHI]:      '🍣', [Resource.SUGAR_CANE]: '🍬',
  [Resource.TIMBER]:     '🪵', [Resource.TOBACCO]:    '🌿', [Resource.TOMATO]:     '🍅',
  [Resource.TOOL]:       '🔧', [Resource.WATCH]:      '⌚', [Resource.WAX]:        '🍯', [Resource.WEAPON]:     '⚔️', [Resource.WHEAT]:      '🌾',
  [Resource.WINDOW]:     '🪟', [Resource.WINE]:       '🍷', [Resource.WOOD]:       '🪵',
  [Resource.WOOL]:       '🐑',
  [Resource.PORCELAIN]:  '🏺', [Resource.LEATHER]:    '👜', [Resource.INCENSE]:    '🌿',
  [Resource.IVORY]:      '🦷', [Resource.SMOKED_FISH]:'🐟', [Resource.FUR_COAT]:   '🧥',
  [Resource.DEER]:       '🦌', [Resource.WHALE_OIL]:  '🛢️',
  [Resource.FERTILIZER]: '🌱',
}

// PNG URL for a resource icon, or undefined to use the emoji fallback.
export function GetResourceIconSrc(type: Resource): string | undefined {
  const file = RESOURCE_ICON_FILES[type]
  return file ? `${RESOURCE_ICON_BASE}${file}` : undefined
}

export function GetResourceEmoji(type: Resource): string {
  return RESOURCE_EMOJI[type] ?? '📦'
}
