// Give jinlin (east) and columbia (west) the main city's (anrelia) grass and
// sea surfaces. Their sand already matches anrelia, so the sand-sea and
// grass-sand blend sets are copied straight from anrelia's (hand-crafted)
// sets; only dirt-grass is regenerated, against each city's own rock tile.
...
async function copyBlendTiles(fromDir, toDir) {
  for (const mask of HEX) await fs.copyFile(`${fromDir}/${mask}.png`, `${toDir}/${mask}.png`);
  for (const extra of ['_preview-4x4.png', '_preview-sample-map.png']) {
    await fs.copyFile(`${fromDir}/${extra}`, `${toDir}/${extra}`).catch(() => {});
  }
}
...
for (const city of ['jinlin', 'columbia']) {
  for (const slot of ['grass', 'sea']) {
    for (const name of ['main.png', 'alternative-1.png']) {
      await fs.copyFile(`${USED}/anrelia/surfaces/${slot}/${name}`, `${USED}/${city}/surfaces/${slot}/${name}`);
    }
    await fs.copyFile(`${USED}/${city}/surfaces/${slot}/main.png`, `${USED}/${city}/${slot}.png`);
  }
  // Identical grass/sand/sea on both sides -> reuse anrelia's blend art as-is.
  await copyBlendTiles(`${USED}/anrelia/blend/sand-sea`, `${USED}/${city}/blend/sand-sea`);
  await copyBlendTiles(`${USED}/anrelia/blend/grass-sand`, `${USED}/${city}/blend/grass-sand`);
  // Rock stays per-city, so dirt-grass is regenerated against the new grass.
  await writeBlendSet(
    `${USED}/${city}/blend/dirt-grass`,
    `${USED}/${city}/surfaces/grass/main.png`,
    `${USED}/${city}/surfaces/rock-grass/main.png`,
    false,
    SEAMS[city],
  );
  console.log(`terrain aligned with anrelia: ${city}`);
}
console.log('done');
